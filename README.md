...
ea, eb = embed(m, [a, b])
print((ea * eb).sum().item())   # ~0.99 — the model recognises them as equivalent
```
...
1. **Insert an identity pair.** `H` followed by another `H` on the same qubit cancels back to the identity. Same for `X·X`, `CNOT·CNOT`, `Z·Z`. We pick a random place in the circuit and insert a pair.
...
The adapters cover the common gates. Anything exotic falls through to a generic "other 1-qubit" or "other 2-qubit" token, which the model treats as a vocabulary fallback. Coverage is best for Qiskit and PennyLane; Cirq is somewhat thinner.
...
- **The model is a similarity heuristic, not a proof.** Two circuits getting cosine 0.99 are *probably* equivalent, but not certainly. For applications where an incorrect answer matters, always verify.
...
```
                 ┌────────────────────────────────────────┐
                 │   QuantumCircuit (qiskit / pennylane)  │
                 └──────────────────┬─────────────────────┘
                                    │
                                    ▼
              ┌─────────────────────────────────────────┐
              │   feat.encode → tokens (gate, q1, q2, p)│
              └──────────────────┬──────────────────────┘
                                 │
                                 ▼
              ┌─────────────────────────────────────────┐
              │   embed lookup: gate, q1, q2 + linear   │
              │   project parameters; sum to one vector │
              │   per token                             │
              └──────────────────┬──────────────────────┘
                                 │
                                 ▼
              ┌─────────────────────────────────────────┐
              │   prepend [CLS] token + pos embeddings  │
              └──────────────────┬──────────────────────┘
                                 │
                                 ▼
              ┌─────────────────────────────────────────┐
              │   3 × TransformerEncoder layers         │
              │   (d=128, 4 heads, GELU, batch_first)   │
              └──────────────────┬──────────────────────┘
                                 │
                                 ▼
              ┌─────────────────────────────────────────┐
              │   read [CLS] hidden state → linear head │
              │   → L2 normalise                        │
              └──────────────────┬──────────────────────┘
                                 │
                                 ▼
                     128-dim unit vector ∈ S¹²⁷
```
